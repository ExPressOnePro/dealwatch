...
class FavoriteController extends Controller
{
    public function index(Request $request): Response
    {
        ...
        $active = (clone $base)
            ->whereNotIn('user_status', Deal::CLOSED_STATUSES)
            ->orderByDesc('updated_at')
            ->get()
            ->map(fn (Deal $deal) => $this->serialize($deal));

        $completed = (clone $base)
            ->where('user_status', Deal::STATUS_COMPLETED)
        ...
        $cancelled = (clone $base)
            ->where('user_status', Deal::STATUS_CANCELLED)
            ->orderByDesc('updated_at')
            ->get()
            ->map(fn (Deal $deal) => $this->serialize($deal));
    ...
    public function destroy(Deal $deal): RedirectResponse
    {
        ...
        $deal->update([
            'is_favorite' => false,
            'user_status' => in_array($deal->user_status, Deal::CLOSED_STATUSES, true)
                ? $deal->user_status
                : Deal::STATUS_NEW,
        ]);
    ...
    public function cancel(Request $request, Deal $deal): RedirectResponse
    {
        ...
        $deal->update([
            'user_status' => Deal::STATUS_CANCELLED,
