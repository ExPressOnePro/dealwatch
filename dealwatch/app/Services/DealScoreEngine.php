...
class DealScoreEngine
{
    ...
    public function evaluate(Listing $listing, int $prepCost = 300, int $riskReserve = 300): ?Deal
    {
        ...
        $valuation = $this->valuation->value($listing, $market, $analysis, $prepCost, $riskReserve);
        $report = $analysis['report'];
        ...
        $score = (int) round(
            $breakdown['discount'] * 0.30
            + $breakdown['profit'] * 0.20
            + $breakdown['liquidity'] * 0.12
            + $breakdown['freshness'] * 0.12
            + $breakdown['private_seller'] * 0.08
            + $breakdown['parse_confidence'] * 0.05
            + $valuation['condition_score'] * 0.08
            + $this->scoreValuationConfidence($valuation['valuation_confidence']) * 0.05
        );

        $score = max(0, min(100, $score));

        $verdict = match (true) {
            $score >= 80 && $profit >= 1500 && $discount >= 12 => 'buy',
            $score >= 60 && $profit >= 800 => 'check',
            default => 'ignore',
        };

        if ($profit < 800 || $discount < 8) {
            $verdict = 'ignore';
            $score = min($score, 55);
        }
    ...
    private function applyIgnoredState(Listing $listing, Deal $deal): Deal
    {
        ...
        if ($deal->user_status !== 'dismissed') {
            $deal->forceFill(['user_status' => 'dismissed'])->save();
        }
