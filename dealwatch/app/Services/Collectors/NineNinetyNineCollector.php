...
class NineNinetyNineCollector
{
    ...
    /** Offer type "Продам / Vând" */
    private const OFFER_TYPE_FILTER_ID = 1;

    private const OFFER_TYPE_SELL = 776;

    /** Site field «Модель» (999.md feature 590) */
    private const SITE_MODEL_FEATURE_ID = 590;
...
GRAPHQL;
     ...
     */
    public function collect(int $limit = 40): array
    {
     ...
     */
    private function searchPage(int $limit, int $skip, ?string $currency = null): array
    {
        $input = [
    ...
    private function mapAd(array $ad): ?array
    {
        ...
        $priceMdl = $priceOriginal !== null
            ? app(\App\Services\CurrencyRateService::class)->toMdl($priceOriginal, $currency)
            : null;

        $listingKind = app(\App\Services\ListingKindClassifier::class)->classify($title, $description);

        $publishedAt = now();
        if (! empty($ad['reseted']) && is_string($ad['reseted'])) {
            try {
                $publishedAt = \Carbon\Carbon::createFromFormat(
                    'Y-m-d H:i:s',
                    $ad['reseted'],
                    'Europe/Chisinau'
                ) ?: \Carbon\Carbon::parse($ad['reseted'], 'Europe/Chisinau');
            } catch (\Throwable) {
                try {
                    $publishedAt = \Carbon\Carbon::parse($ad['reseted'], 'Europe/Chisinau');
                } catch (\Throwable) {
                    $publishedAt = now();
                }
            }
    ...
    private function mergeDetailSources(string $id, ?array $graph, ?array $html): array
    {
        ...
        return array_merge($base, [
            ...
            'price' => $html['price_mdl'] ?? $graph['price'] ?? $graph['price_mdl'] ?? null,
            'price_original' => $html['price_original'] ?? $graph['price_original'] ?? null,
            'price_mdl' => $html['price_mdl'] ?? $graph['price_mdl'] ?? $graph['price'] ?? null,
    ...
    private function fetchDetailFromHtml(string $id): ?array
    {
        ...
        $priceMdl = $price !== null
            ? app(\App\Services\CurrencyRateService::class)->toMdl($price, $currency)
            : null;
    ...
    private function extractPrice(array $ad): array
    {
        ...
        $currency = app(\App\Services\CurrencyRateService::class)->normalizeUnit(is_string($unit) ? $unit : null);
