...
class MarketEvidenceService
{
    ...
    {
        $minutes = (int) config('dealwatch.market.evidence_cache_minutes');
        if ($minutes <= 0) {
            return $this->buildEvidence($price);
        }

        // Ключ включает updated_at: пересчёт рынка сам инвалидирует кеш.
        return Cache::remember(
            'market:evidence:'.$price->id.':'.($price->updated_at?->timestamp ?? 0),
            now()->addMinutes($minutes),
            fn () => $this->buildEvidence($price)
        );
    }
     ...
     */
    private function buildEvidence(MarketPrice $price): array
    {
        $mid = $price->marketMid();
        $basis = $price->basis ?? [];
        ...
        if ($distribution['total_private'] > 0) {
            ...
            $steps[] = [
                'title' => (empty($basis['notes']) ? '6' : '7').'. Сколько объявлений в каких ценах',
    ...
    public function zonesMini(MarketPrice $price, ?int $askPriceMdl = null): ?array
    {
        ...
        $base = Cache::remember($cacheKey, now()->addMinutes(max(1, (int) config('dealwatch.market.evidence_cache_minutes'))), function () use ($price) {
    ...
    private function buildZonesMiniBase(MarketPrice $price): ?array
    {
        ...
        $rows = Listing::query()
            ...
            ->where(function ($q) {
                $q->where('price_mdl', '>=', 300);
            })
            ->select(['price_mdl', 'seller_type'])
            ->limit(800)
            ->get()
            ->map(fn (Listing $l) => [
                'price' => (int) ($l->price_mdl ?? 0),
