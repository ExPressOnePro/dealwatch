...
class MarketPriceRebuilder
{
     ...
     */
    public function rebuild(int $months = 0, int $minSamples = 5): array
    {
        $since = $months > 0
        ...
        foreach ($prices as $price) {
            ...
            $buyMax = (int) round($mid * 0.82);
            $buyMin = (int) round($mid * 0.70);
            ...
            $rationale = sprintf(
                'Рынок построен по %d частным объявлениям 999.md (sell, без перекупов и без «куплю») за %s–%s. Медиана %s MDL, межквартильный диапазон (p25–p75) %s–%s MDL. Магазины и объявления «куплю» исключены. buy_max = 82%% от mid.',
                $filtered->count(),
                $periodFrom ?? '—',
                $periodTo ?? '—',
            ...
            $basis = [
                ...
                'buy_rule' => sprintf('buy_max=%s (82%% mid), buy_min=%s (70%% mid)', number_format($buyMax, 0, '.', ' '), number_format($buyMin, 0, '.', ' ')),
